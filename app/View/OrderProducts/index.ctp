<div class="row">
    <section class="content-header">
      <h1><?php echo __('Order Products'); ?></h1>
    </section>
</div>
<hr>
<div class="row">
	...
	<div class="col-md-12">
		<div class="box">
			<div class="box-body table-responsive no-padding">
				<?php if(empty($orderProducts) && !empty($this->request->query['q'])) : ?>
				...
				<table cellpadding="0" cellspacing="0" class="table table-striped table-hover">
					<thead class="text-primary">
						<tr>						
								<th><?php echo $this->Paginator->sort('order_id', __('Order')); ?></th>
								<th><?php echo $this->Paginator->sort('product_id', __('Product')); ?></th>
								<th><?php echo $this->Paginator->sort('unit_value', __('Unit Value')); ?></th>
...
<?php 					 foreach ($orderProducts as $orderProduct): ?>
							<tr>
										<td>
									<?php echo $this->Html->link($orderProduct['Order']['id'], array('controller' => 'orders', 'action' => 'view', $orderProduct['Order']['id'])); ?>
								</td>
								<td>
									<?php echo $this->Html->link($orderProduct['Product']['name'], array('controller' => 'products', 'action' => 'view', $orderProduct['Product']['id'])); ?>
								</td>
								<td><?php echo h($orderProduct['OrderProduct']['unit_value']); ?>&nbsp;</td>
								<td><?php echo h($orderProduct['OrderProduct']['quantity']); ?>&nbsp;</td>
								<td><?php echo h($orderProduct['OrderProduct']['total']); ?>&nbsp;</td>
								<td> <?php echo $orderProduct['OrderProduct']['state'] == 1 ? __('Activo') : __('Inactivo') ;?> </td>									<td class="td-actions">
									    <a rel="tooltip" href="<?php echo $this->Html->url(array('action' => 'view',$orderProduct['OrderProduct']['id'])); ?>" title="<?php echo __('Ver'); ?>" class="btn btn-info  btn-xs">
									        <i class="fa fa-eye"></i>
									    </a>
									    <a rel="tooltip" href="<?php echo $this->Html->url(array('action' => 'edit',$orderProduct['OrderProduct']['id'])); ?>" title="<?php echo __('Editar'); ?>" class="btn btn-success btn-xs">
									        <i class="fa fa-edit"></i>
									    </a>
									    <a rel="tooltip" href="<?php echo $this->Html->url(array('action' => 'delete',$orderProduct['OrderProduct']['id'])); ?>" title="<?php echo $orderProduct['OrderProduct']['state'] == 1 ? __('Deshabilitar') : __('Habilitar'); ?>" class="btn btn-danger btn-xs changeState">
									    	<?php if($orderProduct['OrderProduct']['state'] == 1): ?>									        <i class="fa fa-times-circle-o"></i>
